...
class JackTokenizer:
    ...
    STRING_CONST = lambda word: (not word) and ("\n" not in word)
    ...
    def get_valid_lines(input_stream: typing.TextIO) -> [str]:
        ...
        for line in input_stream:
            no_comment_line, comment_open = JackTokenizer.comment_handler(line, comment_open)
    ...
    @staticmethod
    def comment_handler(line: str, comment_open: bool) -> tuple[str, bool]:
        current_comment = ""
        ...
        if comment_open:
            no_comment_line, comment_open = JackTokenizer.remove_comments(line, current_comment)

        return no_comment_line, comment_open

    @staticmethod
    def remove_comments(line: str, comment: str) -> tuple[str, bool]:
        no_comment = ""
        comment_open = True
        if comment in JackTokenizer.MULTI_LINE_COMMENTS_SYNTAX.keys():
            no_comment = line.split(comment)[0]

        if JackTokenizer.MULTI_LINE_COMMENTS_SYNTAX[comment] in line:
            comment_open = False

        return no_comment, comment_open
    ...
    def tokens_from_word(self, word: str) -> None:
        ...
        for c in word:
            ...
            if JackTokenizer.KEYWORD(current_token):
                current_token, token_type = self.keyword_token_handler(current_token, token_type)

            elif JackTokenizer.SYMBOL(current_token[-1]):
                current_token, token_type = self.symbol_token_handler(current_token, token_type)
    ...
    def keyword_token_handler(self, token, token_type):
    ...
    def symbol_token_handler(self, token, token_type):
